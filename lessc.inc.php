...
class lessc
{
    ...
    protected function tryImport($importPath, $parentBlock, $out)
    {
        ...
        if (isset($this->allParsedFiles[$this->realPath($realPath)])) {
    ...
    {
        $this->allParsedFiles[$this->realPath($file)] = filemtime($file);
    }
    ...
    {
        $baseImportDir = $this->realPath(isset($this->baseUrlPath) ? $this->baseUrlPath : end($this->importDir));
        $lastImportDir = $this->realPath(reset($this->importDir));
        ...
        if (strpos($cleanUrl, '?') !== false)
            list($cleanUrl, $arguments) = explode('?', $cleanUrl);
        if (strpos($cleanUrl, '#') !== false)
            list($cleanUrl, $anchor) = explode('#', $cleanUrl);

        $urlPath = $this->realPath($lastImportDir . DIRECTORY_SEPARATOR . $cleanUrl);


        if ($urlPath === false) {
            return $url;
        }
        if (strpos($urlPath, 'phar://') === 0) {
            return $urlPath;
        }
        ...
        //Return url
        return str_ireplace(
                $cleanUrl, str_repeat('../', count($baseArray) - $i) . implode('/', array_slice($urlArray, $i)), $url
        );
    }
